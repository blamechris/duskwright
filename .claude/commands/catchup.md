...
Summarize: what shipped recently and whether the latest default-branch run is green. Call out any failing run on the default branch explicitly — that's the "something's on fire" signal. A red `purity` job is the loudest possible signal: it means the invariant regressed, and it outranks every other failure.
...
- **Epic** (the primary grouping): `E0-bootstrap`, `E1-fork-ingest`, `E2-purity`, `E3-rule-engine`,
  `E4-picker`, `E5-treatments`, `E6-coverage`, `E7-hard-sites`, `E8-ui`, `E9-upstream-sync`,
  `E10-distribution`
- **Type:** `bug`, `enhancement`, `purity`, `site-coverage`, `upstream`, `docs`, `test`
- **Priority:** `priority:high`, `priority:medium`, `priority:low`
- **Tier:** `tier:cheap`, `tier:strong` (the model-delegation policy)
- **Provenance:** `from-review` — filed by a review pass rather than planned

Sort epics in order and put `purity` items at the top of whatever epic they sit in: a purity issue
is always P0 regardless of its own priority label.
...
<!-- skill-templates: catchup 3f7f775 2026-08-01 -->
