...
# Workflow is named "CI" (.github/workflows/ci.yml). Jobs: hygiene, lint, build, test, purity.
...
```bash
# `purity` is the long pole and gates the whole run: it installs Chromium and runs the
# Playwright corpus, measured at ~560s across recent runs. Every other job finishes in
# under 90s (build ~63s, lint ~38s, test ~24s, hygiene ~12s, e2e ~7s). Keep MAX_WAIT well
# above the purity figure — a limit at or below it times out on every green run.
...
Duskwright-specific patterns:
- `purity` job red → **never retrigger, never exclude.** The gate is correct and the code is
  wrong. Escalate to a fix; a purity regression is the most serious failure this repo has.
- `attribution footer found` (hygiene) → real policy violation in content, OR the check is
  matching prose that merely names the policy. Check whether the hit is a line-start trailer
  before assuming the former.
- `missing required file` (hygiene) → a tracked doc was deleted or renamed. FIX, never retrigger.
- `Co-Authored-By trailer found` → rewrite the commit; do not weaken the check.
- `npm ci` lockfile mismatch → FIX (commit the lockfile), not transient.
...
# Fallback: close/reopen the PR, or push an empty commit to the branch.
# `workflow_dispatch` is not wired on this workflow.
```
...
```bash
# See the timeout note above — raise once the purity harness installs Chromium.
MAX_WAIT=300
INTERVAL=30
ELAPSED=0
...
<!-- skill-templates: fix-ci af9c1e4 2026-08-03 -->
