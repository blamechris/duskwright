...
Origin: PR #${SOURCE_PR} review. For a purity finding, lead with that and classify it P0 (serializable into the page's document model) / P1 (observable by page JS) / P2 (destroys page-owned state) — purity outranks everything else on the board.
...
# Duskwright has no complexity: labels. It uses priority:high|medium|low for urgency and
# tier:cheap|strong for the model-delegation policy. Map --complexity onto tier: (low -> tier:cheap,
# medium|high -> tier:strong), and add the epic label (E0-bootstrap ... E10-distribution) when known.
# Anything touching page-owned DOM also gets `purity` and priority:high.
